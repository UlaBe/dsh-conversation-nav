...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// Tick rail: a picker-wheel track. The rail is a 9-tick-high window;
			// the track holds ALL ticks and is translated so the ACTIVE tick
			// sits near the middle. The blue highlight IS the active tick (a
			// color/size transition = "handoff" between neighbours); the track
			// transition duration adapts to the jump size: small steps animate
			// (smooth handoff), large jumps snap instantly (the target tick is
			// deep in the track, an animation would drag the highlight through
			// the window).
			".convnav-tick-rail{position:relative;width:40px;height:340px;overflow:hidden}",
			".convnav-tick-track{position:absolute;top:0;left:0;right:0;display:flex;flex-direction:column;align-items:center;gap:28px;transition:transform 250ms cubic-bezier(.3,.55,.45,1);will-change:transform}",
			...
			// Diagnostics text: kept in the DOM (same render path as the
			// working numbered build) but hidden.
			".convnav-diag{margin-top:4px;text-align:center;font-size:9px;color:#6b6e74;pointer-events:none;display:none}",
		 ...
		 * into noise). Manual "promote" and the model's topic marker both
		 * correct the rare mis-groupings.
		 */
		...
		/** The token the model appends when it judges a message starts a new topic. */
		const TOPIC_MARK = "<!--new_topic-->";

		/** Concatenated VISIBLE text of an assistant message (reasoning excluded:
		 *  the model mentions the token inside its own reasoning, which must not
		 *  count as a boundary). */
		...
		/**
		 * Collect user-message indexes that the model marked as new-topic roots.
		 * Walks the node stream: each assistant message carrying the topic token
		 * marks the most recent user message as a root.
		 */
		function llmRootsOf(snapshot) {
			const roots = new Set();
			if (!snapshot || !Array.isArray(snapshot.nodes)) return roots;
			let userIndex = -1;
			for (const n of snapshot.nodes) {
				if (!n) continue;
				if (n.kind === "user") {
					userIndex++;
				} else if (n.kind === "assistant" && userIndex >= 0) {
					if (assistantTextOf(n).trimEnd().endsWith(TOPIC_MARK)) roots.add(userIndex);
				}
			}
			return roots;
		}
		 ...
		 */
		function buildTopicTree(userMessages, manualRoots, llmRoots, timeThreshold) {
			const roots = [];
			...
			for (let i = 0; i < userMessages.length; i++) {
				...
				const isRoot = manualRoots.has(i) || llmRoots.has(i) || isNewTopicRoot(msg, prev, timeThreshold);
				if (isRoot) {
					currentRoot = i;
					roots.push(i);
					children.set(i, []);
			...
			return function NavIndicator(props) {
				...
				react.useEffect(() => {
					...
					const update = () => {
						...
						// Active = the last row whose top is above the 40% line of
						// the visible viewport (more forgiving than a fixed
						// headroom, so mid-session jumps land correctly).
						const vp = scroller.getBoundingClientRect();
						const mid = vp.top + vp.height * 0.4;
						let active = 0;
						for (let i = 0; i < rows.length; i++) {
							if (rows[i].getBoundingClientRect().top <= mid) active = i;
							else break;
						}
						// When scrolled to the very bottom, the last message IS the
						// visible target even though its top may sit below the
						// midpoint (long reply above it pushes it down).
						if (scroller.scrollTop + scroller.clientHeight >= scroller.scrollHeight - 8) {
							active = rows.length - 1;
						}
						setActiveIndex(active);
						setDomRowCount(rows.length);
						// TEMP diagnostics (remove after the mid-session highlight
						// issue is confirmed fixed).
						if (typeof console !== "undefined") {
							console.log("[nav] active=" + active + " rows=" + rows.length + " users=" + userMessages.length + " blue=" + (active >= 0 ? active * tickSpacing - Math.max(0, Math.min(active * tickSpacing - (390 - tickSpacing) / 2, Math.max(0, userMessages.length * tickSpacing - 390))) : "n/a"));
						}
					};
					const onScroll = () => {
				...
				const jump = (index) => {
					// Set the highlight immediately — do not wait for the scroll
					// spy to infer the target from the DOM.
				...
				// TEMP diagnostics: DOM user-row count vs data count (remove later)
				const [domRowCount, setDomRowCount] = react.useState(-1);
				const panelRef = react.useRef(null);
				const prevHovered = react.useRef(false);
				const RAIL_H = 340; // window height, matched to the panel
				// When the panel opens, position the current message so its
				// vertical slot in the panel matches the blue line's slot in
				// the tick rail (e.g. blue line at slot 6 => the current
				// message is also at ~slot 6 in the panel viewport).
				react.useEffect(() => {
					if (hovered && !prevHovered.current && panelRef.current && activeIndex >= 0) {
						const panel = panelRef.current;
						const row = panel.querySelector('[data-index="' + activeIndex + '"]');
						if (row instanceof HTMLElement) {
							const spacing = tickSpacing;
							const max = Math.max(0, userMessages.length * spacing - RAIL_H);
							const off = Math.max(0, Math.min(activeIndex * spacing - (RAIL_H - spacing) / 2, max));
							const bluePos = activeIndex * spacing - off;
							const ratio = RAIL_H > 0 ? bluePos / RAIL_H : 0.5;
							const target = row.offsetTop - panel.clientHeight * ratio + row.offsetHeight / 2;
							panel.scrollTop = Math.max(0, target);
						}
					}
					prevHovered.current = hovered;
				}, [hovered, activeIndex, tickSpacing, userMessages.length]);
				const llmRoots = react.useMemo(() => llmRootsOf(snapshot), [snapshot]);
				const tree = react.useMemo(
					() => buildTopicTree(userMessages, manualRoots, llmRoots, TIME_THRESHOLD),
					[userMessages, manualRoots, llmRoots]
				);
				...
				// Tick rail: a full-length track scrolled like a picker wheel.
				// The active tick is kept near the middle of the 9-tick window;
				// at the ends the offset clamps so the first/last tick settles
				// at the window edge with a smooth transition. The per-tick
				// spacing is MEASURED at runtime (not assumed) — a wrong
				// constant accumulates error and pushes the active tick out of
				...
				const railOffset = Math.max(0, Math.min(targetOffset, maxOffset));
				// Track animation adapts to the jump size: small steps slide
				// smoothly (the blue highlight hands off between neighbours);
				// large jumps snap so the target tick (deep in the track) lands
				// in the window instantly instead of dragging through it.
				const prevActiveRef = react.useRef(-1);
				const delta = activeIndex >= 0 && prevActiveRef.current >= 0 ? Math.abs(activeIndex - prevActiveRef.current) : 0;
				prevActiveRef.current = activeIndex;
				const trackDuration = delta > 4 ? "0ms" : "250ms";
						...
						: react.createElement(
								...
								react.createElement(
									...
									react.createElement(
										...
										{
											ref: trackRef,
											className: "convnav-tick-track",
											style: { transform: "translateY(" + -railOffset + "px)", transitionDuration: trackDuration }
										},
								...
								react.createElement(
									"div",
									{ className: "convnav-diag" },
									"u" + userMessages.length + " r" + domRowCount + " a" + activeIndex
								)
							),
					hovered && tooltip
						? react.createElement(
