...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".convnav-ticks{display:flex;flex-direction:column;align-items:center;gap:9px;padding:12px 8px;border-radius:12px}",
			...
			".convnav-panel{position:fixed;right:8px;top:50%;transform:translateY(-50%);width:272px;background:var(--dsw-alias-bg-raised,var(--dsw-alias-bg-base,#232428));border:1px solid var(--dsw-alias-border-l1,#2e2f33);border-radius:14px;box-shadow:0 8px 28px rgba(0,0,0,.40);padding:8px;max-height:318px;overflow-y:auto;-webkit-mask-image:linear-gradient(to bottom,transparent,#000 14px,#000 calc(100% - 14px),transparent);mask-image:linear-gradient(to bottom,transparent,#000 14px,#000 calc(100% - 14px),transparent);animation:convnav-pop .18s cubic-bezier(.4,0,.2,1)}",
