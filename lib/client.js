...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".convnav-panel{position:fixed;right:8px;width:240px;background:#34363d;border:1px solid #46484f;border-radius:14px;box-shadow:0 8px 28px rgba(0,0,0,.45);padding:8px;max-height:390px;overflow-y:auto;-webkit-mask-image:linear-gradient(to bottom,transparent,#000 14px,#000 calc(100% - 14px),transparent);mask-image:linear-gradient(to bottom,transparent,#000 14px,#000 calc(100% - 14px),transparent);animation:convnav-pop .18s cubic-bezier(.4,0,.2,1)}",
			...
			// Full-message tooltip: lighter card matching the panel, shown on the
			// LEFT of the panel and vertically aligned with the hovered row.
			// Only long messages trigger it; the text is excerpted by a fixed
			// max-height with a bottom fade (no scrollbar — it cannot be used
			// anyway since leaving the panel dismisses the tip).
			".convnav-tip{position:fixed;right:262px;max-width:280px;max-height:132px;overflow:hidden;background:#34363d;border:1px solid #46484f;border-radius:10px;box-shadow:0 6px 20px rgba(0,0,0,.45);padding:8px 12px;font-size:13px;line-height:1.5;color:var(--dsw-alias-label-primary,#e6e7ea);white-space:pre-wrap;word-break:break-word;-webkit-mask-image:linear-gradient(to bottom,#000 0,#000 calc(100% - 18px),transparent);mask-image:linear-gradient(to bottom,#000 0,#000 calc(100% - 18px),transparent);transform:translateY(-50%);pointer-events:none;z-index:2147483002;animation:convnav-tip-in .15s cubic-bezier(.4,0,.2,1)}",
		...
		function makeNavIndicator(ctx) {
			...
			return function NavIndicator(props) {
				...
				// Tooltip trigger: only for messages whose preview is truncated
				// (length > 10 chars, matching the preview threshold), and only
				// after the pointer has settled on the row for 400ms — a quick
				// pass over the panel never pops the tip.
