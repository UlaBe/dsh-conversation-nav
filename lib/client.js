		...
		function makeNavIndicator(ctx) {
			return function NavIndicator(props) {
				...
				const jump = (index) => {
					...
					// Center the window on the jump target so the blue line
					// lands mid-window.
					setWinStart(Math.max(0, Math.min(index - Math.floor(SLOT_COUNT / 2), Math.max(0, userMessages.length - SLOT_COUNT))));
				...
				// Tick rail: a paging list window (9 slots, pure React, no
				// transforms). The blue line moves within the window as active
				// changes (0.2s color handoff between slots); when active exits
				// the window, the window pages by one slot. Jumps center the
				// window. This gives visible movement AND nothing that can
				// detach from state.
				const SLOT_COUNT = 9;
				...
				react.useEffect(() => {
					...
					setWinStart((ws) => {
						...
						if (activeIndex >= ws + SLOT_COUNT) return activeIndex - (SLOT_COUNT - 1);
						return ws;
					});
				}, [activeIndex]);
				const slots = [];
				{
					...
					const w = Math.max(0, Math.min(winStart, Math.max(0, len - SLOT_COUNT)));
