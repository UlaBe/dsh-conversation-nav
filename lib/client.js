...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// The blue line's top = the active tick's target window position:
			// 72px (middle) when not clamped, sliding to 0/144 at the ends via
			// a CSS top transition. Decoupled from the track, so it is always
		...
		function makeNavIndicator(ctx) {
			return function NavIndicator(props) {
				...
				// Independent blue line: its top = the active tick's TARGET
				// window position (72px middle when unclamped, 0..72 near the
				// start, 144..72 near the end). Decoupled from the track's
				// in-flight position, so it is always correct and visible.
				const blueTop =
					activeIndex >= 0 ? activeIndex * tickSpacing - railTargetRef.current : -100;
