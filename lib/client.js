...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Grey tick width graded by message length, so the rail's window
		 *  content visibly changes as it scrolls past different messages. */
		function tickWidthOf(text) {
		...
		function makeNavIndicator(ctx) {
			return function NavIndicator(props) {
				...
				// Tick rail: 9 fixed slots, pure React, no transforms. The window
				// is CENTERED on the active index, so the blue line always sits
				// in the middle slot (at the edges it clamps to the first/last
				// slot). Grey ticks are width-graded by message length, so the
				// window content visibly changes as it scrolls past different
				// messages — movement is readable while the highlight stays put.
				const SLOT_COUNT = 9;
				const slots = [];
				{
									...
									idx === null
										...
										: react.createElement("button", {
												...
												style:
													idx === activeIndex
														? undefined
														: { width: tickWidthOf(textOf(userMessages[idx])) + "px" },
												onClick: () => jump(idx),
