...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// pointer-events none: the zone is LAYOUT ONLY. Hover is handled by
			// the rail/panel themselves — so the panel only opens when the
			// pointer is actually on the rail, and scrolling the chat with the
			// pointer anywhere else keeps the rail (blue line) visible.
			...
			// Tick rail: window (162px = 9 ticks) with a full-length grey track
			// (rAF-translated for content scroll) and an INDEPENDENT blue line
			// whose top = the active tick's target window position (72px middle
			// when unclamped, sliding to 0/144 at the ends). Decoupled from the
			// track, so it is always correct and always visible.
		...
		function makeNavIndicator(ctx) {
			return function NavIndicator(props) {
				...
				// window, translated so the active tick sits in the middle. The
				// transform is driven by an always-on rAF loop reading a target
				// ref (synced every render) — independent of React renders and
				// CSS animations, so it can never go stale. Jump-locked or very
				// large offsets snap; manual small steps ease.
				...
				// Sync the track target every render and derive the independent
				// blue line's top (target window position of the active tick):
				// 72px (middle) when unclamped, 0..72 near the start, 144..72
				// near the end. Decoupled from the track's in-flight transform.
				const RAIL_H = 162;
				...
				const blueTop = activeIndex >= 0 ? activeIndex * tickSpacing - railTargetRef.current : -100;
