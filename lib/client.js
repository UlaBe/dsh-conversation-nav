...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// the window).
			".convnav-tick-rail{position:relative;width:40px;height:380px;overflow:hidden}",
			".convnav-tick-track{position:absolute;top:0;left:0;right:0;display:flex;flex-direction:column;align-items:center;gap:34.7px;transition:transform 250ms cubic-bezier(.3,.55,.45,1);will-change:transform}",
		...
		function makeNavIndicator(ctx) {
			return function NavIndicator(props) {
				...
				const RAIL_H = 380; // window height, matched to the panel
