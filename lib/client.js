...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ── styles ──────────────────────────────────────────────────────────────
		const css = [
			// Hover capture zone: LAYOUT ONLY (pointer-events none). Hover is
			// handled by the rail/panel themselves, so scrolling the chat with
			// the pointer anywhere else keeps the rail (blue line) visible.
			".convnav-zone{position:fixed;right:8px;top:0;bottom:0;display:flex;align-items:center;z-index:2147483000;pointer-events:none}",
			// Tick rail: window (162px) with a full-length grey track and an
			// INDEPENDENT blue line. Both the track transform AND the blue
			// line's top are written every frame by the rAF loop (never via
			// React renders), so React bailout cannot stale them.
			".convnav-tick-rail{position:relative;width:40px;height:162px;overflow:hidden;pointer-events:auto}",
			...
			".convnav-tick-center{position:absolute;left:50%;transform:translateX(-50%);width:28px;height:3px;border-radius:3px;background:#4f6ef2;box-shadow:0 0 6px rgba(79,110,242,.5);z-index:1;pointer-events:none}",
		...
		function makeNavIndicator(ctx) {
			return function NavIndicator(props) {
				...
				// Tick rail: full-length track + independent blue line, both
				// driven by one always-on rAF loop that writes DOM styles
				// directly every frame — completely independent of React
				// renders/bailout, which was the root cause of the highlight
				// disappearing. Blue line top follows the active tick's live
				// track position (72px = middle when unclamped, 0/144 at the
				// ends), kept in sync with the track in the same loop.
				...
				const spacingRef = react.useRef(18);
				const userMessagesRef = react.useRef(0);
				const [tickSpacing, setTickSpacing] = react.useState(18);
				spacingRef.current = tickSpacing;
				activeRef.current = activeIndex;
				userMessagesRef.current = userMessages.length;
				react.useEffect(() => {
				...
				react.useEffect(() => {
					...
					const loop = () => {
						...
						// Recompute the clamped target every frame from live refs.
						const max = Math.max(0, userMessagesRef.current * spacing - RAIL_H);
						const target = Math.max(
							0,
							Math.min(active * spacing - (RAIL_H - spacing) / 2, max)
						);
						railTargetRef.current = target;
						if (track) {
							const diff = railTargetRef.current - railCurRef.current;
							if (lockedRef.current || Math.abs(diff) > 72) {
						...
						if (center) {
							const top = active >= 0 ? active * spacing - railCurRef.current : -100;
				...
				return react.createElement(
					...
					hovered
						...
						: react.createElement(
								"div",
								{ className: "convnav-tick-rail", onMouseEnter: enter, onMouseLeave: leave },
								react.createElement("div", { ref: centerRef, className: "convnav-tick-center" }),
								react.createElement(
									"div",
