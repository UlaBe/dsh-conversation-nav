...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".convnav-zone{position:fixed;right:8px;top:50%;transform:translateY(-50%);z-index:2147483000}",
			// Tick rail (resting): thin grey ticks, active one brand-blue & longer.
			".convnav-ticks{display:flex;flex-direction:column;align-items:center;gap:5px;padding:10px 6px;border-radius:10px}",
			".convnav-tick{width:12px;height:2px;border-radius:2px;border:none;background:#56585f;opacity:.6;cursor:pointer;padding:0;transition:width .2s ease,background-color .2s ease,opacity .2s ease}",
			".convnav-tick:hover{opacity:.95}",
			".convnav-tick.convnav-active{width:20px;background:#4f6ef2;opacity:1}",
			...
			// wheel-scrolls for more.
			".convnav-panel{position:fixed;right:8px;top:50%;transform:translateY(-50%);width:216px;background:var(--dsw-alias-bg-raised,var(--dsw-alias-bg-base,#232428));border:1px solid var(--dsw-alias-border-l1,#2e2f33);border-radius:12px;box-shadow:0 8px 28px rgba(0,0,0,.40);padding:6px;max-height:264px;overflow-y:auto;animation:convnav-pop .18s cubic-bezier(.4,0,.2,1)}",
			...
			".convnav-row{display:flex;align-items:center;gap:8px;width:100%;padding:5px 4px 5px 10px;border-radius:8px;border:none;background:none;color:var(--dsw-alias-label-secondary,#b9bcc2);cursor:pointer;text-align:left;font:inherit;font-size:12px;line-height:1.3;transition:color .2s ease,background-color .2s ease}",
			...
			".convnav-row-tick{flex:none;width:12px;height:2px;border-radius:2px;background:currentColor;opacity:.85}",
