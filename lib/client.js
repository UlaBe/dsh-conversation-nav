...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// sits near the middle. The blue line is a SEPARATE element pinned
			// to the active tick's window position (not a class on a tick), so
			// large jumps slide the tick column under a stationary blue line
			// instead of moving the highlight out of the window.
			".convnav-tick-rail{position:relative;width:40px;height:162px;overflow:hidden}",
			".convnav-tick-track{position:absolute;top:0;left:0;right:0;display:flex;flex-direction:column;align-items:center;gap:15px;transition:transform .25s cubic-bezier(.3,.55,.45,1);will-change:transform}",
			...
			".convnav-tick-center{position:absolute;left:50%;transform:translateX(-50%);width:28px;height:3px;border-radius:3px;background:#4f6ef2;box-shadow:0 0 6px rgba(79,110,242,.5);z-index:1;pointer-events:none;transition:top .25s cubic-bezier(.3,.55,.45,1)}",
		...
		function makeNavIndicator(ctx) {
			return function NavIndicator(props) {
				...
				// Window position of the blue highlight line: it follows the
				// active tick's rendered position (72px = middle when not
				// clamped; 0 / bottom at the ends). It is a separate element,
				// so the tick column slides beneath it on jumps.
				const blueTop = activeIndex >= 0 ? activeIndex * tickSpacing - railOffset : -100;
					...
					hovered
						...
						: react.createElement(
								...
								react.createElement("div", {
									className: "convnav-tick-center",
									style: { top: blueTop + "px" }
								}),
								react.createElement(
									"div",
									{ ref: trackRef, className: "convnav-tick-track", style: { transform: "translateY(" + -railOffset + "px)" } },
									userMessages.map((m, i) =>
										react.createElement("button", {
											key: m.seq,
											className: "convnav-tick",
											onClick: () => jump(i),
