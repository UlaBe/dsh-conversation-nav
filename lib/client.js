	...
	factory: (require) => {
		...
		function makeNavIndicator(ctx) {
			return function NavIndicator(props) {
				...
				react.useEffect(() => {
					...
					const update = () => {
						...
						if (typeof console !== "undefined") {
							console.log("[nav] active=" + active + " rows=" + rows.length + " users=" + userMessages.length + " blue=" + (active >= 0 ? active * tickSpacing - Math.max(0, Math.min(active * tickSpacing - (162 - tickSpacing) / 2, Math.max(0, userMessages.length * tickSpacing - 162))) : "n/a"));
						}
					};
					const onScroll = () => {
				...
				const RAIL_H = 162; // 9 ticks visible in the window
