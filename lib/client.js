...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// Tick rail: N fixed slots rendered purely by React — the blue
			// highlight is simply the slot whose tick is active. No transform,
			// no animation, nothing that can detach from state: the active tick
			// is always rendered in its slot (middle of the window, or at the
			// edges when clamped at the ends).
			".convnav-tick-rail{display:flex;flex-direction:column;align-items:center;gap:15px;width:40px;height:162px;overflow:hidden}",
			".convnav-tick{width:18px;height:3px;border-radius:3px;border:none;background:#56585f;opacity:.65;cursor:pointer;padding:0;transition:width .2s ease,background-color .2s ease,opacity .2s ease;animation:convnav-tick-in .2s ease-out}",
			...
			".convnav-tick.convnav-tick-empty{visibility:hidden}",
			// One-shot entrance animation: a freshly mounted tick slides in from
			// one slot above (picker-wheel feel). It is a pure CSS animation on
			// mount — the final position is the flex slot, so nothing can drift.
			"@keyframes convnav-tick-in{from{transform:translateY(18px)}to{transform:translateY(0)}}",
		...
		function makeNavIndicator(ctx) {
			return function NavIndicator(props) {
				...
				// Tick rail: 9 fixed slots rendered purely by React. The window
				// is centered on the active index (clamped at the ends); the
				// active tick is always the one in its slot, so the blue
				// highlight can never disappear — no transforms involved.
				const SLOT_COUNT = 9;
				const slots = [];
				{
					...
					const winStart = Math.max(
						0,
						Math.min(
							activeIndex === -1 ? 0 : activeIndex - Math.floor(SLOT_COUNT / 2),
							Math.max(0, len - SLOT_COUNT)
						)
					);
					for (let k = 0; k < SLOT_COUNT; k++) {
						const idx = winStart + k;
						slots.push(idx < len ? idx : null);
					}
				}
				...
				return react.createElement(
					...
					hovered
						...
						: react.createElement(
								...
								slots.map((idx, k) =>
									idx === null
										? react.createElement("span", { key: "empty-" + k, className: "convnav-tick convnav-tick-empty" })
										: react.createElement("button", {
												key: userMessages[idx].seq,
												className: "convnav-tick" + (idx === activeIndex ? " convnav-active" : ""),
												onClick: () => jump(idx),
												"aria-label": "跳转到第 " + (idx + 1) + " 条发言"
										  })
								)
							),
					hovered && tooltip
