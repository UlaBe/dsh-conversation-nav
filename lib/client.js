...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// sits near the middle, clamping smoothly at the ends.
		...
		function makeNavIndicator(ctx) {
			return function NavIndicator(props) {
				...
				// at the window edge with a smooth transition.
				const TICK_ITEM_H = 18; // 3px tick + 15px gap
				const RAIL_H = 9 * TICK_ITEM_H; // 162px
				const maxOffset = Math.max(0, userMessages.length * TICK_ITEM_H - RAIL_H);
				const targetOffset = activeIndex * TICK_ITEM_H - (RAIL_H - TICK_ITEM_H) / 2;
				...
				return react.createElement(
					...
					hovered
						...
						: react.createElement(
								...
								react.createElement(
									"div",
									{ className: "convnav-tick-track", style: { transform: "translateY(" + -railOffset + "px)" } },
