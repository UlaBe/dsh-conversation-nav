	...
	factory: (require) => {
		...
		function makeNavIndicator(ctx) {
			return function NavIndicator(props) {
				...
				const snapTrack = delta > 4;

				// The track transform is driven by a JS rAF loop, NOT by a CSS
				// transition: CSS transitions can be interrupted/left mid-flight
				// (and React bails out when setState values are unchanged), which
				// used to leave the track detached from the active tick so the
				// highlight drifted out of the window. With rAF the transform is
				// written every frame and can never go stale.
				const railOffsetRef = react.useRef(0);
				const railAnimRef = react.useRef(null);
				react.useEffect(() => {
					const track = trackRef.current;
					if (!track) return;
					if (snapTrack) {
						railOffsetRef.current = railOffset;
						track.style.transform = "translateY(" + -railOffset + "px)";
						return;
					}
					if (railAnimRef.current) cancelAnimationFrame(railAnimRef.current);
					const step = () => {
						const cur = railOffsetRef.current;
						const diff = railOffset - cur;
						if (Math.abs(diff) < 0.5) {
							railOffsetRef.current = railOffset;
							track.style.transform = "translateY(" + -railOffset + "px)";
							railAnimRef.current = null;
							return;
						}
						railOffsetRef.current = cur + diff * 0.22; // ease-out
						track.style.transform = "translateY(" + -railOffsetRef.current + "px)";
						railAnimRef.current = requestAnimationFrame(step);
					};
					railAnimRef.current = requestAnimationFrame(step);
					return () => {
						if (railAnimRef.current) cancelAnimationFrame(railAnimRef.current);
					};
				}, [railOffset, snapTrack]);
