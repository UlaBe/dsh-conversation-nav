/**
 * @ulabe/dsh-conversation-nav — conversation position navigator (flat list).
 *
 ...
 *   slightly longer.
 * - Hover: the rail expands into a rounded grey panel — every user message
 *   listed flat, each with its preview on the left. Up to 9 rows are
 ...
 *
 * Flat-list variant: no topic tree (no grouping, no collapse, no promote).
 *
									...
									: userMessages.map((m, i) => {
											...
											return react.createElement(
												"button",
												{
													key: m.seq,
													"data-index": i,
													className: "convnav-row" + (active ? " convnav-active" : ""),
													onClick: () => jump(i),
